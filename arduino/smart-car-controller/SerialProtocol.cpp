...
int SerialProtocol::recvData()
{
    ...
    if (receivedPacket->Check())
    {
        if (receivedPacket->packet[0] == 0xFA && receivedPacket->packet[1] == 0xFB)
        {
            ...
            else if (receivedPacket->packet[3] == ENABLE
                            || receivedPacket->packet[3] == MOVE
                            || receivedPacket->packet[3] == SERVO
                            || receivedPacket->packet[3] == ULTRASONIC
                            || receivedPacket->packet[3] == JOY_INFO
                            || receivedPacket->packet[3] == STOP
                            || receivedPacket->packet[3] == AP_CREATED
                            || receivedPacket->packet[3] == AP_FAILED
                            || receivedPacket->packet[3] == AP_CONNECTED
                            || receivedPacket->packet[3] == AP_DISCONNECTED)
            {
                data_received = true;
                return 0;
            }
